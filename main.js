...
var sortedFiles = derived([sort, files], ([$sort, $files]) =>
	[...$files].sort((a, b) => {
		switch ($sort) {
...
var preparedSearch = derived(searchQuery, ($searchQuery) =>
	$searchQuery ? (0, import_obsidian.prepareFuzzySearch)($searchQuery) : null,
);
...
MiniMasonry.prototype.layout = function () {
	...
	for (var index = 0; index < children2.length; index++) {
		...
		children2[index].style.transform =
...
function create_if_block(ctx) {
	...
	return {
		...
		m(target, anchor) {
			...
			if (!mounted) {
				dispose = action_destroyer(
					(folderIcon_action = /*folderIcon*/ ctx[6].call(null, span0)),
				);
				mounted = true;
			}
		},
		p(ctx2, dirty) {
			if (
				dirty /*file*/ & 1 &&
				t_value !== (t_value = /*file*/ ctx2[0].parent.path + "")
			)
				set_data(t, t_value);
		},
		d(detaching) {
	...
	return {
		...
		m(target, anchor) {
			...
			if (!mounted) {
				dispose = [
					action_destroyer(
						(trashIcon_action = /*trashIcon*/ ctx[5].call(null, button)),
					),
					listen(
						button,
						"click",
		...
		p(new_ctx, [dirty]) {
			...
			if (
				dirty /*file*/ & 1 &&
				t0_value !== (t0_value = /*file*/ ctx[0].basename + "")
			)
			...
			if (dirty /*$skipNextTransition*/ & 16) {
				toggle_class(
					div2,
					"skip-transition",
					/*$skipNextTransition*/
...
function instance($$self, $$props, $$invalidate) {
	...
	component_subscribe($$self, skipNextTransition, ($$value) =>
		$$invalidate(4, ($skipNextTransition = $$value)),
	);
	let { file } = $$props;
	let { openFile } = $$props;
	...
	let { trashFile } = $$props;
	let contentDiv;
	const trashIcon = (element2) =>
		(0, import_obsidian3.setIcon)(element2, "trash");
	const folderIcon = (element2) =>
		(0, import_obsidian3.setIcon)(element2, "folder");
...
function add_css2(target) {
	append_styles(
		target,
		"svelte-1nuqxjz",
		".action-bar.svelte-1nuqxjz{display:flex;flex-direction:row;justify-content:end;gap:var(--size-4-5);margin-bottom:var(--size-4-5)}.cards-container.svelte-1nuqxjz{position:relative;container-type:inline-size}.cards-container.svelte-1nuqxjz *{--card-padding:var(--size-4-3);--card-gutter:var(--size-4-5)}",
	);
}
...
function create_each_block(key_1, ctx) {
	...
	return {
		...
		p(new_ctx, dirty) {
			ctx = new_ctx;
			const card_changes = {};
			if (dirty /*$displayedFiles*/ & 128) card_changes.file = /*file*/ ctx[23];
			if (dirty /*renderFile, $displayedFiles*/ & 129)
				card_changes.renderFile = func;
			if (dirty /*openFile, $displayedFiles*/ & 130)
				card_changes.openFile = func_1;
			if (dirty /*trashFile, $displayedFiles*/ & 132)
...
function create_fragment2(ctx) {
	...
	const get_key = (ctx2) =>
		/*file*/
		ctx2[23].path /*file*/ + ctx2[23].stat.mtime;
	...
	return {
		...
		m(target, anchor) {
			...
			if (!mounted) {
				dispose = [
					action_destroyer(
						(searchInput_action = /*searchInput*/ ctx[9].call(null, div0)),
					),
					action_destroyer(
						(sortIcon_action = /*sortIcon*/ ctx[8].call(null, button)),
					),
					listen(
						button,
						"click",
		...
		p(ctx2, [dirty]) {
			if (
				dirty /*$displayedFiles, renderFile, openFile, trashFile, notesGrid*/ &
				143
			) {
...
function instance2($$self, $$props, $$invalidate) {
	...
	let $displayedFiles;
	component_subscribe($$self, skipNextTransition, ($$value) =>
		$$invalidate(19, ($skipNextTransition = $$value)),
	);
	component_subscribe($$self, viewIsVisible, ($$value) =>
		$$invalidate(20, ($viewIsVisible = $$value)),
	);
	component_subscribe($$self, sort, ($$value) =>
		$$invalidate(21, ($sort = $$value)),
	);
	component_subscribe($$self, searchQuery, ($$value) =>
		$$invalidate(22, ($searchQuery = $$value)),
	);
	component_subscribe($$self, displayedFiles, ($$value) =>
		$$invalidate(7, ($displayedFiles = $$value)),
	);
	let { settings } = $$props;
	let { renderFile } = $$props;
	let { openFile } = $$props;
		...
		sortMenu2.addItem((item) => {
			...
			item.onClick(() =>
				__awaiter(this, void 0, void 0, function* () {
					...
					$$invalidate(
						11,
						(settings.defaultSort = "Title (A-Z)") /* NameAsc */,
						settings,
					);
					yield saveSettings();
				}),
		...
		sortMenu2.addItem((item) => {
			...
			item.onClick(() =>
				__awaiter(this, void 0, void 0, function* () {
					...
					$$invalidate(
						11,
						(settings.defaultSort = "Title (Z-A)") /* NameDesc */,
						settings,
					);
					yield saveSettings();
				}),
		...
		sortMenu2.addItem((item) => {
			...
			item.onClick(() =>
				__awaiter(this, void 0, void 0, function* () {
					set_store_value(
						sort,
						($sort = "Edited (Newest First)") /* EditedDesc */,
						$sort,
					);
					$$invalidate(
						11,
						(settings.defaultSort = "Edited (Newest First)") /* EditedDesc */,
						settings,
					);
					yield saveSettings();
				}),
		...
		sortMenu2.addItem((item) => {
			...
			item.onClick(() =>
				__awaiter(this, void 0, void 0, function* () {
					set_store_value(
						sort,
						($sort = "Edited (Oldest First)") /* EditedAsc */,
						$sort,
					);
					$$invalidate(
						11,
						(settings.defaultSort = "Edited (Oldest First)") /* EditedAsc */,
						settings,
					);
					yield saveSettings();
				}),
		...
		sortMenu2.addItem((item) => {
			...
			item.onClick(() =>
				__awaiter(this, void 0, void 0, function* () {
					set_store_value(
						sort,
						($sort = "Created (Newest First)") /* CreatedDesc */,
						$sort,
					);
					$$invalidate(
						11,
						(settings.defaultSort = "Created (Newest First)") /* CreatedDesc */,
						settings,
					);
					yield saveSettings();
				}),
		...
		sortMenu2.addItem((item) => {
			...
			item.onClick(() =>
				__awaiter(this, void 0, void 0, function* () {
					set_store_value(
						sort,
						($sort = "Created (Oldest First)") /* CreatedAsc */,
						$sort,
					);
					$$invalidate(
						11,
						(settings.defaultSort = "Created (Oldest First)") /* CreatedAsc */,
						settings,
					);
					yield saveSettings();
				}),
	...
	async onOpen() {
		...
		this.registerEvent(
			this.app.vault.on("create", async (file) => {
				if (file instanceof import_obsidian5.TFile && file.extension === "md") {
					store_default.files.update((files2) =>
						files2 == null ? void 0 : files2.concat(file),
					);
				}
			}),
		);
			...
			this.app.vault.on("delete", async (file) => {
				if (file instanceof import_obsidian5.TFile && file.extension === "md") {
					store_default.files.update((files2) =>
						files2 == null
							? void 0
							: files2.filter((f) => f.path !== file.path),
					);
				}
			}),
		);
			...
			this.app.vault.on("modify", async (file) => {
				if (file instanceof import_obsidian5.TFile && file.extension === "md") {
					store_default.files.update((files2) =>
						files2 == null
							? void 0
							: files2.map((f) => (f.path === file.path ? file : f)),
					);
				}
			}),
		);
			...
			this.app.vault.on("rename", async (file, oldPath) => {
				if (file instanceof import_obsidian5.TFile && file.extension === "md") {
					store_default.files.update((files2) =>
						files2 == null
							? void 0
							: files2.map((f) => (f.path === oldPath ? file : f)),
					);
				}
			}),
		);
		viewContent.addEventListener("scroll", async () => {
			if (
				...
				store_default.displayedCount.set(
					get_store_value(store_default.displayedFiles).length + 50,
				);
			}
		});
		this.app.workspace.on("active-leaf-change", () => {
			...
			store_default.viewIsVisible.set(
				((_a = rootLeaf == null ? void 0 : rootLeaf.view) == null
					? void 0
					: _a.getViewType()) === VIEW_TYPE,
			);
		});
	}
...
var CardsViewPlugin2 = class extends import_obsidian6.Plugin {
		...
		this.registerDomEvent(document, "click", (evt) => {
			...
			if (this.settings.openCardsViewOnTagTreeClick) {
				...
				if (tagElement) {
					...
					if (textElement) {
						const tagName =
							(_a = textElement.textContent) == null ? void 0 : _a.trim();
						if (tagName) {
		...
		this.registerDomEvent(document, "click", (evt) => {
			...
			if (this.settings.openCardsViewOnInlineTagClick) {
				...
				if (
					...
					const tagName =
						(_a = target.textContent) == null ? void 0 : _a.trim();
					if (tagName) {
						this.openTagInCardsView(tagName);
						evt.preventDefault();
					}
	...
	async openAllFilesInFolder(folder) {
		const files2 = folder.children.filter(
			(child) =>
				child instanceof import_obsidian6.TFile && child.extension === "md",
		);
	...
	async openTagInCardsView(tagName) {
		try {
			...
			if (leaves.length > 0) {
				const cardsView = leaves[0].view;
				cardsView.updateFiles(
					files2,
					"Created (Newest First)" /* CreatedDesc */,
				);
			} else {
			...
			new import_obsidian6.Notice(
				`\u6253\u5F00\u5361\u7247\u89C6\u56FE\u65F6\u51FA\u9519: ${this.getErrorMessage(error)}`,
			);
		}
	}
	async getFilesWithTag(tagName) {
		...
		for (const file of this.app.vault.getMarkdownFiles()) {
			...
			if (
				(_a = cache == null ? void 0 : cache.tags) == null
					? void 0
					: _a.some((tag) => tag.tag === `#${tagName}`)
			) {
				files2.push(file);
			}
		}
